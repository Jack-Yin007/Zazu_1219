...
static bool mp2762a_i2c_write_byte(uint8_t reg, uint8_t val)
{
    ...
    ret = pf_i2c_0_write(MP2762A_I2C_ADDR, reg, &val, 1);
...
static bool mp2762a_i2c_read_byte(uint8_t reg, uint8_t *p_data)
{
    ...
    ret = pf_i2c_0_read(MP2762A_I2C_ADDR, reg, p_data, 1);
...
#define MP2762A_CHG_CUR_LIMIT		1000	// Unit in mA
...
void mp2762a_init(uint8_t enable_temp)
{
...
static void mp2762a_ntc_sample_proc(bool init_call)
{
	...
	if (no_external_power() == true)
	{
		...
		temp_ntc = get_temp(0);
		monet_data.temp_ntc = temp_ntc;
		if (monet_data.temp_ntc == 125) 
		{                               
			monet_data.temp_ntc = 0;
		}
		monet_data.temp = monet_data.temp_nordic;
		NRF_LOG_RAW_INFO("ERROR: failed to read NTC resistance No source.\r");
		return;
	}
	...
	if (bat_ntc_res_get(&ntc_res) != true)
	{
		...
		monet_data.temp = monet_data.temp_nordic;
		NRF_LOG_RAW_INFO("ERROR: failed to read NTC resistance fail.\r");
		return;
	}
	...
	if (temp <= CHG_TEMP_LIMIT1)
	{
		...
		temp_disable_cnt_44 = 0;
	}
	else if (temp <= CHG_TEMP_LIMIT2)
	{
		chg_cur_target = (mp2762a_deflt_chg_cur_needed == true) ? CHG_CUR_LIMIT400 :
						chg_cur_cali_lr(CHG_CUR_LIMIT0, CHG_CUR_LIMIT400, CHG_CUR_LIMIT1000, chg_cur_settting, temp, CHG_TEMP_LIMIT1, CHG_TEMP_LIMIT2);
		temp_disable_cnt_29++;
	...
	{
		chg_cur_target = (mp2762a_deflt_chg_cur_needed == true) ? CHG_CUR_LIMIT1000 :
						chg_cur_cali_lr(CHG_CUR_LIMIT400, CHG_CUR_LIMIT1000, CHG_CUR_LIMIT400, chg_cur_settting, temp, CHG_TEMP_LIMIT2, CHG_TEMP_LIMIT3);
		temp_disable_cnt_44++;				
		if (temp_disable_cnt_44 > 3)
		{
			temp_disable = 0;
		}
		//temp_disable_cnt_29 = 0;
	...
	else if (temp < monet_data.temp_set)
	{
		// chg_cur_target = (mp2762a_deflt_chg_cur_needed == true) ? CHG_CUR_LIMIT400 :
		// 				chg_cur_cali_lr(CHG_CUR_LIMIT1000, CHG_CUR_LIMIT400, CHG_CUR_LIMIT0, chg_cur_settting, temp, CHG_TEMP_LIMIT3, CHG_TEMP_LIMIT4);
		chg_cur_target = (mp2762a_deflt_chg_cur_needed == true) ? CHG_CUR_LIMIT400 :
						chg_cur_cali_lr(CHG_CUR_LIMIT1000, CHG_CUR_LIMIT400, CHG_CUR_LIMIT0, chg_cur_settting, temp, CHG_TEMP_LIMIT3, monet_data.temp_set);
		temp_disable_cnt_59++;
	...
	else
	{
		...
		chg_cur_target = (mp2762a_deflt_chg_cur_needed == true) ? CHG_CUR_LIMIT0 :
							chg_cur_cali_r(CHG_CUR_LIMIT400, CHG_CUR_LIMIT0, chg_cur_settting, temp, monet_data.temp_set);
	...
	if (chg_cur_target == CHG_CUR_LIMIT0 && charge_en == MP2762A_CHG_EN_ENABLE)
	{
		...
		// NRF_LOG_RAW_INFO("mp2762a_ntc_sample_proc. Outside Temp range suspend charging \r");
	...
	monet_data.temp_ntc = temp_ntc;
	monet_data.temp_nordic = temp_nordic;
	// if (monet_data.temp_ntc == 125) //Warning: we found if power off the charger ntc would return this value.
	// {                               //this value can be clear 0 manually. 
	// 	monet_data.temp_ntc = 0;
	// }
	if (monet_data.temp_ntc == 125) //Warning: we found if power off the charger ntc would return this value.
	{                               //this value can be clear 0 manually. 
		monet_data.temp_ntc = monet_data.temp_nordic;
	}
	// monet_data.temp_ntc = -55;
	// monet_data.temp_nordic = -25;  // for test.
	monet_data.temp = temp;
...
void mp2762a_proc(void)
{
	...
	monet_data.temp_nordic = get_temp(1);
