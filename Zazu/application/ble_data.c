...
void ble_data_send_with_queue(void)
{
    ...
    if ((is_ble_data_queue_empty(&ble_data_tx_queue) == 1) || (get_tx_queue_number() == 1))
    {
    ...
    while (!is_ble_data_queue_empty(&ble_data_tx_queue) && !retry)
    {
        ...
        err_code = ble_aus_data_send_periheral(m_ble_send_buf.p_data, length, channel);
        //pf_log_raw(atel_log_ctl.platform_en, "the queue ble %d \r",m_ble_send_buf.p_data[0]);
        ...
        {
            if (get_ble_conn_handle() == BLE_CONN_HANDLE_INVALID)
            {
                if ((monet_data.ble_disconnnet_delayTimeMs >= (6 * 1000)) || (monet_data.cameraPowerOnDelay < (3 * 1000)))
                {
                    pf_log_raw(atel_log_ctl.platform_en, "Delete the queue ble disconnect %d %d %d %d\r",m_ble_send_buf.p_data[0],
                                                        ble_data_tx_queue.num,monet_data.ble_disconnnet_delayTimeMs,monet_data.cameraPowerOnDelay);

                    if (m_ble_send_buf.p_data)
                    {
                        if (m_ble_send_buf.p_data[0] != 112)  //camera ready
                        {
                            ble_data_queue_delete_one(&ble_data_tx_queue);
                            m_ble_send_buf.p_data = NULL;
                            m_ble_send_buf.length = 0;
                    ...
                    // m_len_sent += length;
                    retry = false;
                }        // In app logic, if disconnected once, this task would stop. so we need to delete this scenario ble queue data
                else
                {
                    break;
                }                                                                            
            ...
            {
                monet_data.ble_disconnnet_delayTimeMs = 0;
                pf_log_raw(atel_log_ctl.platform_en, "Repeat the queue ble connect %d %d %d\r",m_ble_send_buf.p_data[0],ble_data_tx_queue.num,get_ble_conn_handle());
                if (monet_data.mcuDataHandleMs <= 1500 && monet_data.cameraPowerOn == 0) // now,esp32 off and the ble exist,but can't send successfully,delete the queue.
                {                                                                        // Due to P0 esp32 off,  add this case delete the queue.
                    if (m_ble_send_buf.p_data)
                    {
                        ble_data_queue_delete_one(&ble_data_tx_queue);
                        m_ble_send_buf.p_data = NULL;
                    ...
                    retry = false;
                    pf_log_raw(atel_log_ctl.platform_en, "Delete the queue ble connect  %d\r",ble_data_tx_queue.num);
                }
                else
                {
                    retry = true;
...
void ble_send_data_rate_show(uint8_t force)
{
         ...
         force)
    {
        ...
        if ((m_len_sent >= BLE_DISCONNECT_DATA_RATE_THRESHOLD) || 
            (m_len_recv >= BLE_DISCONNECT_DATA_RATE_THRESHOLD))
        {
        ...
        if ((index >= 5) || force)
        {
            ...
            for (i = 0; i < index; i++)
            {
                pf_log_raw(atel_log_ctl.platform_en,"==**Speed Tx: %d B/s Rx: %d B/s(%d)**==\r", 
                           data_send_rec[i], data_recv_rec[i], ble_data_throughput_count - index + i);
            }
            index = 0;
            pf_log_raw(atel_log_ctl.platform_en,"ble_send_result channel(%d) status(%d) error(%d)\r", 
                           monet_data.ble_send_result.channel, monet_data.ble_send_result.status, monet_data.ble_send_result.error);
            CRITICAL_REGION_ENTER();
            monet_data.ble_send_result.error = 0;
            CRITICAL_REGION_EXIT();
        }
...
static void ble_send_timer_handler(void * p_context)
{
    ...
    if (monet_data.mcuDataHandleMs >= BLE_DATA_SEND_TIMER_PERIOD_MS)
    {
        monet_data.mcuDataHandleMs -= BLE_DATA_SEND_TIMER_PERIOD_MS;
    }
    monet_data.ble_disconnnet_delayTimeMs += BLE_DATA_SEND_TIMER_PERIOD_MS;
    CRITICAL_REGION_EXIT();
...
int8_t ble_send_data_pop(uint8_t **p_data, uint16_t *p_len, uint16_t *p_ch)
{
    int8_t err_code;
    buffer_t buf = {0};
    ...
    err_code = ble_data_queue_out(&ble_data_tx_queue, &buf);
    if (err_code)
    {
        NRF_LOG_ERROR("ble_send_data_pop pop fail.");
        return -2;
    }
...
void ble_send_data_delete_one(void)
{
    ble_data_queue_delete_one(&ble_data_tx_queue);
}
...
uint16_t get_tx_queue_number(void)
{
    return ble_data_tx_queue.num;
}
