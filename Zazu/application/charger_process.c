...
void charger_power_on(uint8_t mode)
{
    if (pf_gpio_read(GPIO_CHRG_SLEEP_EN) != CHARGER_CHIP_WAKE_VALUE) // The charger is disabled
    {
...
void setChargerOn1(void)   //Do not read Temp with charger On.
{
    //	GPIO_PinOutClear(CHG_EN_PORT, CHG_EN_PIN);
    charger_power_on(0);
    mp2762a_charge_ctrl(true);
...
void solar_chg_mode_proc(void)
{
	...
	if (flag_s_v_check == true)
	{
		...
		if (!s_v_check_debounce)
		{
            ...
            //temp = get_temp(1);
            charger_printf("solar_chg_mode_proc(), temp:(%d) \r", monet_data.temp_nordic);
            if((monet_data.temp_nordic > monet_data.temp_set) || (monet_data.temp_nordic <  CHG_TEMP_LIMIT1))
            {
                monet_data.charge_mode_disable_nrf = 1;
                solar_chg_mode_select(FUNC_JUMP_POINT_4);

            }
            else
            {
                monet_data.charge_mode_disable_nrf = 0;
...
void solar_chg_mode_select(uint8_t jump_point)
{
    ...
    if (monet_data.charge_mode_disable || monet_data.charge_mode_disable_nrf)   
    {
    ...
    {
        if (exist_main_power() == true) // zazumcu-129. Main exist do not in mode2.
        {
            jump_point = FUNC_JUMP_POINT_6;
        }
        else
        {
...
//			break;	// Comment this line to avoid warning
        case FUNC_JUMP_POINT_6:
			goto func_jump_point_6;
//			break;	// Comment this line to avoid warning
		default:
			break;
	}
...
func_jump_point_2:
		...
		if (sol_vol > VOL_LIMIT_S_CHG_MODE_SEL)
		{
			...
			{
                if ((monet_data.charge_mode_disable == 1) || (monet_data.charge_mode_disable_nrf == 1))
                {
                    solar_chg_mode_set(SOLAR_CHG_MODE1);
                    setChargerOn1();
                    setChargerOff();
			...
			{
func_jump_point_3:
                if ((monet_data.charge_mode_disable == 1) || (monet_data.charge_mode_disable_nrf == 1))   //if real time, do not need add. here as the special cased, we add this.
                {
                    solar_chg_mode_set(SOLAR_CHG_MODE1);
                    setChargerOn1();
                    setChargerOff();
...
func_jump_point_5: 
        {
            if (monet_data.charge_mode1_disable)
            {
                if ((monet_data.charge_mode_disable == 1) || (monet_data.charge_mode_disable_nrf == 1))
                {
                    // solar_chg_mode_set(SOLAR_CHG_MODE1);  // do not need. Due to delete the mppt jump function.
                    // setChargerOn1();
                    // setChargerOff();
                    return;
                }
...
void solar_chg_mode_check_proc(void)
{
    ...
    if((monet_data.temp_nordic > monet_data.temp_set) || (monet_data.temp_nordic <  CHG_TEMP_LIMIT1))
    {
        ...
        charger_printf("The temp out normal range,choose to off. Temp (%d).\r",monet_data.temp_nordic);
    }
    else
    {
        monet_data.charge_mode_disable_nrf = 0;
	    ...
	    charger_printf("solar_chg_mode_check_proc(), timer triggered.\r");
        charger_printf("The temp in healthy range, normal choose the mode. Temp (%d).\r",monet_data.temp_nordic);
    }
...
//  static uint8_t restart_indicate_debounce = RESTART_INDICATE_COUNT;
    ...
    //if (monet_data.V3PowerOn == 0)
    {
        ...
        if (AdcBackup_mv < BUB_THRESHOLD_CHANGE_TEMP)
        {
            ...
            || (only_solar_power() == true && is_solar_vol_low() == true))
        {
            ...
            // else
            {
                if (no_external_power() == true)
                {
                    ...
                    //     timer_solar_chg_mode_restart();
                    //     //timer_solar_chg_mode_stop();
                    //     monet_data.charge_status = CHARGE_STATUS_NO_SOURCES_MODE_2;
                    // }
                    //else
                    {
